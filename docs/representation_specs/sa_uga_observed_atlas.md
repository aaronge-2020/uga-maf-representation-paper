...
- UGA representation labels and atlas-status metadata in `src/utils/label_registry.json`.
...
Atlas construction and event encoding are independent of endpoint labels. Observed-atlas variants are treated as sensitivity analyses unless they are explicitly promoted into the production protocol.
