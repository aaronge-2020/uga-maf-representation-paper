...
def generate_repro_manifest() -> None:
    payload = {
        ...
        "active_directories": ["src", "config", "data", "results/tables", "results/manuscript", "docs", "manifests"],
        "science_guards": [
...
def validate_required_manuscript_files(datasets_dir: Path | None = None) -> dict[str, object]:
    asset_manifest = read_json(ROOT / "results/manuscript/manuscript_asset_manifest.json")
    if not isinstance(asset_manifest, dict):
        raise RuntimeError("manuscript asset manifest is not a JSON object")
    required = [str(path) for path in asset_manifest.get("required_files", [])]
    backing = dataset_backing_lookup() if datasets_dir is not None else {}
    missing = []
    dataset_backed = 0
    for path in required:
        asset_path = manuscript_asset_path(path)
        if asset_path.exists():
            continue
        repo_relative = rel(asset_path)
        backing_row = backing.get(repo_relative)
        if backing_row and datasets_dir is not None:
            validate_dataset_backing(repo_relative, backing_row, datasets_dir)
            dataset_backed += 1
            continue
        missing.append(path)
    if missing:
        raise RuntimeError(f"missing required manuscript files: {missing[:20]}")
    endpoints = set(map(str, asset_manifest.get("main_endpoints", [])))
    required_endpoints = {"cancer_type_top20", "hrd_binary_24", "hrd_binary_33", "hrd_binary_42"}
    if not required_endpoints.issubset(endpoints):
        raise RuntimeError(f"main endpoint manifest missing {sorted(required_endpoints - endpoints)}")
    return {"required_files": len(required), "dataset_backed_required_files": dataset_backed, "main_endpoints": sorted(endpoints)}


def validate_main_results() -> dict[str, object]:
    rows = read_csv(ROOT / "results/manuscript/canonical/main_panel_results.csv")
    ...
    if not needed.issubset(endpoints):
        raise RuntimeError(f"canonical main panel missing endpoints: {sorted(needed - endpoints)}")
    top20 = [row for row in rows if row.get("endpoint") == "cancer_type_top20"]
    if not top20:
        raise RuntimeError("canonical main panel missing cancer_type_top20 rows")
    if not any(str(row.get("n_samples", "")).startswith("8800") for row in top20):
        raise RuntimeError("cancer_type_top20 canonical rows do not report n_samples=8800")
    bad_metric = [row for row in top20 if str(row.get("metric", "")).strip() != "balanced_accuracy"]
    if bad_metric:
        raise RuntimeError("cancer_type_top20 canonical rows must use metric=balanced_accuracy")
    return {"canonical_rows": len(rows), "endpoints": sorted(endpoints)}


def validate_bio_maf_outputs() -> dict[str, object]:
    required = [
        "results/manuscript/tables/table_s4_cancer_type_top20_metrics.csv",
        "results/manuscript/tables/table_s5_bio_maf_v4_feature_guide.csv",
        "results/manuscript/tables/table_s6_bio_maf_v4_nested_feature_selection.csv",
        "results/manuscript/tables/publication/table_s4_cancer_type_top20_metrics.csv",
        "results/manuscript/tables/publication/table_s5_bio_maf_v4_feature_guide.csv",
        "results/manuscript/tables/publication/table_s6_bio_maf_v4_nested_feature_selection.csv",
        "results/manuscript/supplement/table_s4_cancer_type_top20_metrics.csv",
        "results/manuscript/supplement/table_s5_bio_maf_v4_feature_guide.csv",
        "results/manuscript/supplement/table_s6_bio_maf_v4_nested_feature_selection.csv",
        "results/manuscript/supplement/figure_s4_bio_maf_v4_feature_map.png",
        "results/manuscript/supplement/figure_s4b_bio_maf_v4_feature_distributions.png",
        "results/manuscript/supplement/figure_s4c_bio_maf_v4_feature_distributions_by_cancer_type.png",
        "results/manuscript/supplement/figure_s5_bio_maf_v4_nested_feature_selection.png",
        "results/manuscript/interpretability/bio_maf_v4_interpretability_manifest.json",
    ]
    missing = [path for path in required if not (ROOT / path).exists()]
    if missing:
        raise RuntimeError(f"missing Bio MAF v4 S4/S5/S6 outputs: {missing}")
    s6 = read_csv(ROOT / "results/manuscript/tables/table_s6_bio_maf_v4_nested_feature_selection.csv")
    if not s6:
        raise RuntimeError("table S6 is empty")
    header = set(s6[0])
    needed_columns = {"Mean inner-validation score", "Mean outer-fold score", "Outer folds selected", "Outer folds evaluated"}
    if not needed_columns.issubset(header):
        raise RuntimeError(f"table S6 missing nested-CV columns: {sorted(needed_columns - header)}")
    ...
    return {"s6_rows": len(s6), "bio_maf_v4_required_outputs": len(required)}
...
def validate_strict(dataset_validation: dict[str, object] | None = None, datasets_dir: Path | None = None) -> dict[str, object]:
    ...
    manuscript_manifest = validate_manifest_rows(
        load_manifest_csv("manuscript_artifact_manifest.csv"),
        require_exact=True,
        base=ROOT / "results/manuscript",
    ...
    manuscript = validate_required_manuscript_files(datasets_dir)
    ...
    payload = {
        ...
        "bio_maf_v4": bio_maf,
        "manifests": {
            "manuscript_artifact_manifest": manuscript_manifest,
