...
EXTENDED_REL_TARGETS = [
    "docs/poster/generated_figures",
]


def _resolve_targets(paths: dict, *, include_extended: bool) -> list[Path]:
    ...
    for path in configured:
        ...
        try:
            ...
            raise RuntimeError(f"Cleanup target is outside repository root: {resolved}") from exc
        targets.append(resolved)
    if include_extended:
        targets.extend((REPRO_ROOT / rel).resolve() for rel in EXTENDED_REL_TARGETS)
...
def clean(paths: dict, *, execute: bool, include_extended: bool) -> dict[str, object]:
    targets = _resolve_targets(paths, include_extended=include_extended)
    rows: list[dict[str, object]] = []
    for target in targets:
...
def main() -> None:
    ...
    parser.add_argument("--include-extended", action="store_true", help="Also clear approved generated report/result/scratch folders outside the core production outputs.")
    ...
    manifest = clean(paths, execute=bool(args.execute), include_extended=bool(args.include_extended))
    if args.manifest:
    ...
    print(json.dumps({k: manifest[k] for k in ["mode", "include_extended", "target_count", "artifact_count", "total_bytes"]}, indent=2), flush=True)
    if not args.execute:
