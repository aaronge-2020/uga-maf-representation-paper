...
DATASETS_ROOT = Path(os.environ.get("CGR_DATASETS_DIR", BUNDLE_ROOT.parent / "github_exports_datasets")).expanduser()
...
def resolve_path(path: str | Path | None, *, base: Path | None = None) -> Path | None:
    ...
    - `${repro_root}` for this self-contained export root.
    - `${datasets_root}` for the sibling/external dataset bundle.
    ...
    text = (
        ...
        .replace("${datasets_root}", str(DATASETS_ROOT))
